...
<!-- begin:: Content -->
									...
									<div class="kt-portlet__head-toolbar">
										<div class="kt-portlet__head-wrapper">
											<div class="kt-portlet__head-actions">
												&nbsp;
												<a href="{{ route('admin.help.create') }}" class="btn btn-brand btn-elevate btn-icon-sm">
													 <i class="la la-bullseye"></i>
													New Help & Support
												</a>
											</div>
	...
	<tr role="row">
		...
		@foreach($help as $index => $_help)
			<tr>
				<td>{{ ++$index  }}</td>
				<td>{{ $_help->helpCategory->title ?? '' }}</td>
				<td>{{ Str::limit($_help->title ?? '',10) }}</td>
				<td>{{ Str::limit($_help->description ?? '',10) }}</td>
				<td>{{ $_help->type == 1?'Seller':'Buyer' }}</td>
				<td>
					<a title="Edit" href="{{ route('admin.help.edit',[$_help->id]) }}" class="btn btn-sm btn-clean btn-icon btn-icon-sm"> 
